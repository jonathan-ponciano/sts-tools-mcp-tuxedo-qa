...
- `start_pair_debug` / `get_pair_debug_context` / `stop_pair_debug` — opens a visible browser for a human to drive by hand while console/network/errors/actions get recorded with timestamps; use when the user wants to walk through a flow live and have you spot the bug, or wants a draft test built from what they just did, instead of writing a scripted test upfront. The dashboard's Monitor tab also mirrors the session live (a low-fps screenshot feed) — mention that if the user isn't sitting where the browser window itself opened.
